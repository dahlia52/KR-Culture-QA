...
def parse_arguments():
    ...
    parser.add_argument('--output', type=str, default=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                         'resource/QA/split_by_type/korean_culture_qa_선다형_to_서술형.json'),
...
def main():
    ...
    pipe, tokenizer = load_llm(model_id=args.model_id, device=args.device, quantize=args.quantize, batch_size=args.batch_size)
    if not pipe:
